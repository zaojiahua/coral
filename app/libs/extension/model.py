...
from app.execption.inner.total import InstanceExistError, InstanceNotExistError
from app.libs.extension.filter import obj_filter_by_field, field_key_without_pk_filter
        ...
        for arg in dir(cls):
            ...
            if isinstance(val, fields.ModelField):
                if isinstance(val, fields.BaseHash):
                    ...
                    for filter_fields in redis_client.keys(  # todo:keys会造成阻塞，须更换成scan，但会出现重复值
