    ...
    def __getattr__(self, item):
        ...
        def _method_wrapper(*args, **kwargs):
            ...
            for v in args:
                ...
                new_args.append(
                    v.pk if isinstance(v, models.Model) else json.dumps(v) if isinstance(v, dict) else v)
                for k, v in kwargs.items():
                    new_kwargs[k] = v.pk if isinstance(v, models.Model) else json.dumps(v) if isinstance(v,
                                                                                                         dict) else v
            ...
            if item in self._single_object_answered_redis_methods:
                return None if not answer else json.loads(answer) if self._to == dict else self._to(answer)
                    ...
                    if not pk:
                        ...
                        if isinstance(pk, tuple) and len(pk) > 0:
                            wrapper_answer.append(
                                (json.loads(pk[0]) if self._to == dict else self._to(pk[0]), pk[1]))
                        else:
                            wrapper_answer.append(
                                json.loads(pk) if self._to == dict else self._to(pk))
...
class FloatValidatorMixin:
    def _validate(self, value):
        if not isinstance(value, float):
            raise ValueError('Invalid type of field %s: %s. Expected is float' %
                             (self._name, type(value).__name__))
        return value
